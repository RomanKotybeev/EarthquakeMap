...
public class ParseFeed {
	...
	public static HashMap<String, Float> loadCounrtyGDPFromCSV(PApplet p, String fileName) {
		// HashMap key: country ID and  data: lifeExp at birth
		HashMap<String, Float> CounrtyGDPMap = new HashMap<String, Float>();
		...
		for (String row : rows) {
			 ...
			 */
			//
			int data2017 = columns.length - 2;
			
			
				// check if value exists for year
			if(!columns[data2017].equals("..") && !columns[data2017].equals("2017y")) {
				CounrtyGDPMap.put(columns[1], Float.parseFloat(columns[data2017]));
			}
		}
		System.out.println(CounrtyGDPMap);

		return CounrtyGDPMap;
	}
